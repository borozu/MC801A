  ...
  $.ajax({
    ...
    success: function (sig) {
      for (
        ...
        window[vars[i]] = sig[vars[i]];
      if (
        ($("#nr5rsrpb")
          .parent()
          .toggle("" != sig.nr5g_action_band),
        $("#Z5g_SINR")
          .parent()
          .toggle("" != sig.nr5g_action_band),
...
function barGraph(p, val, min, max) {
  for (
    ...
    (pax = (gt + 1) * (x + 1)),
      (pay = gh - 1),
      (pby = gh - ((window[ap][x] - min) / (max - min)) * gh),
      (pc = ((window[ap][x] - min) / (max - min)) * 100),
      pc < 50
        ? (color = "yellow")
        : pc < 85
        ? (color = "green")
...
function lockcell(p, e) {
  $.ajax({
    ...
    success: function (data) {
      (ad = hex_md5(
        hex_md5(data.wa_inner_version + data.cr_version) + data.RD
      )),
        $.ajax({
          type: "POST",
...
function ltebandselection() {
  ...
  if ((band && (band = band.toLowerCase()), null != band && "" !== band)) {
    var bs = band.split("+"),
      ltesum = 0;
    if (((all_bands = "0xA3E2AB0908DF"), "AUTO" === band.toUpperCase()))
      ltesum = all_bands;
    else {
